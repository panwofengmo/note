...
local start_time = time.time('2023-11-09 12:00:00')
local end_time = time.time('2023-11-10 01:00:00')
...
function check_coin_log(path, account_map)
	...
	if file then
		while true do
			...
			if not account_map or (account_map and account_map[account]) then 
				...
				local change = tonumber(values[7]) local source = values[9]
				local log_time = tonumber(values[10])
				local unique_room_id = tonumber(values[11])
				local room_type = values[12]
				if source and string.find(source, "金币场带", 1, true) and log_time >= start_time and log_time <= end_time then
					...
					if source == "金币场带出" and tg == 0 then
						tg = 1
					end

					table.insert(info, {tg, account, change, source, log_time, unique_room_id, room_type})
				end
			end
		end
	end
	...
	for account, obj in pairs(map) do
		for unique_room_id, info in pairs(obj) do
			...
			local room_type = last_info[7]
			if tg == 0 and room_type == "truco" then
				if #info > 1 then
					for i = #info, 1, -1 do
						local o = info[i]
						local t = o[1]
...
-- check_coin_log("/logdata/2023-11-10/coins.log")
-- check_coin_log("/root/redis_envt/test_coins.log")
...
-- check_silver_log("/root/redis_envt/silver_20231019.log")
