...
local function logic()
	...
	local ids = util.lines_of("silver_bonus_id.txt", "\n", true)
	for i = 1, #ids do
		local id = tonumber(ids[i])
		local key = "dpzj:silver_bet_bonus_data:" .. id
		local kvs = redis:hgetall(key)
		for k, v in pairs(kvs) do
			if string.find(k, "real_") then
				local json = cjson.decode(v)
				local item_bonus = json["item_bonus"] or 0
				local vip_bonus = json["vip_bonus"] or 0
				if item_bonus > 0 or vip_bonus > 0 then
					json["item_bonus"] = 0
					json["vip_bonus"] = 0
					local newv = cjson.encode(json)
					redis:hset(key, k, newv)
					print("fixedbonus", id, k, v)
				end
			end
		end
	end
