...
function get_day_after(date, before)
	...
	local sql = string.format("select %s from daily_diamond_details where date='%s' limit 1", field, time.date(query_date, true))
...
function one_day(date, before)
	...
	local sql = string.format("insert into daily_diamond_details(`date`, `origin`, `add`, `use`, `last`) values('%s', %s, %s, %s, %s)", date, bigint_tostring(bg_origin), bigint_tostring(bg_add), bigint_tostring(bg_use), bigint_tostring(bg_last))
