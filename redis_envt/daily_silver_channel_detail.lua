...
function one_day(date)
	...
	while true do
		...
		if source ~= "分销商发放扣除" and source ~= "分销商发放获得" then
			...
			if type == 0 then
				...
				if source == "分销商资源兑换消耗" or source == "silver兑换钻石" then
	...
	for channel, use in pairs(channels) do
		if use[0] ~= 0 or use[1] ~= 0 then
			...
			local sql = string.format("insert into daily_silver_channel_details(`date`, `channel`, `silver`, `exchange`) values('%s', %d, %s, %s)", date, channel, bigint_tostring(use), bigint_tostring(exchange))
