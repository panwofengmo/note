...
local function sort_request(client, command, key, params)
    ...
    return request.multibulk(client, command, query)
end
...
local function zset_range_request(client, command, ...)
    ...
    return request.multibulk(client, command, args)
end
...
local function mset_filter_args(client, command, ...)
    ...
    return request.multibulk(client, command, arguments)
end
...
local function parse_cluster(client, response, command)
	...
	response:gsub('([^\n]*)\n', function(kv)
		...
		local result = {}
		string.gsub(kv, '[^ ]+', function(w) table.insert(result, w) end)

		local node_name = result[1]
		local host_and_port  = result[2]
		local flags = result[3]

		local host_and_port_index = string.find(host_and_port, ":")
		local host = string.sub(host_and_port, 1, host_and_port_index - 1)
		local cluster_port_index = string.find(host_and_port, "@")
		cluster_port_index = cluster_port_index and cluster_port_index - 1
		local port = tonumber(string.sub(host_and_port, host_and_port_index + 1, cluster_port_index))

		local is_master = string.find(flags, "master") ~= nil
		if is_master then
			local is_myself = string.find(flags, "myself") ~= nil

			local node_slots = {}
			local slots_index_min = 9
			local slots_index_max = #result
			for slots_index_current = slots_index_min, slots_index_max do
				local slots = result[slots_index_current]
				local slots_index = string.find(slots, "-")
				local slot_begin = nil
				local slot_end = nil
				if slots_index then
					slot_begin = tonumber(string.sub(slots, 1, slots_index - 1))
					slot_end = tonumber(string.sub(slots, slots_index + 1))
				else
					slot_begin = tonumber(slots)
					slot_end = slot_begin
				end
				for slot_num = slot_begin, slot_end do
					node_slots[slot_num] = true
				end
			end
			...
			node.is_myself = is_myself
			node.slots = node_slots
	...
	for _, node in pairs(nodes) do
		local r = client
		if not node.is_myself then
			r = Redis.connect(node.host, node.port, client.password, false, client.timeout)
		end
		node.crc16 = client.crc16
		node.redis = r
		node.main_redis = client
		table.insert(cluster_redis, node)
	end
	client.cluster_redis = cluster_redis

	local cluster_slots = {}
	for _, node in pairs(nodes) do
		local node_slots = node.slots
		for slot_num, _ in pairs(node_slots) do
			cluster_slots[slot_num] = node
		end
	end
	client.cluster_slots = cluster_slots
end
...
local function scan_request(client, command, ...)
    ...
    return request.multibulk(client, command, req)
end
...
function response.read(client)
   ...
   -- error reply
    elseif prefix == '-' then
		if string.find(data, "MOVED") == 1 then
        	return client.error('redis moved error: ' .. data)
		else
...
function request.multibulk(client, command, ...)
	...
	if client.cluster_support and client.cluster_redis and command ~= "SCAN" and command ~= "SSCAN" and command ~="ZSCAN" and command ~= "HSCAN" and args[1] then
		local key_target_slots = client.crc16(args[1], #args[1])
		local node = client.cluster_slots[key_target_slots]
		real_client = node.redis
	end

	real_client.network.write(real_client, table.concat(buffer))
	return real_client
end

-- ############################################################################

local function custom(command, send, parse, need_client)
    command = string.upper(command)
    return function(client, ...)
		if client.cluster_support and command == "DEL" then
			local args = {...}
			local argsn = #args
			if argsn > 1 then
				local cluster_client = client
				local result = 0
				for i = 1, argsn do
					client = send(cluster_client, command, args[i])
        ...
        if type(reply) == 'table' and reply.queued then
            ...
            if parse then
				if need_client then
                	return parse(client, reply, command, ...)
				else
...
local function command(command, opts)
    if opts == nil or type(opts) == 'function' then
        ...
        return custom(command, opts.request or request.multibulk, opts.response, opts.need_client)
    end
end
...
client_prototype.quit = function(client)
	local cluster_redis = client.cluster_redis
	if cluster_redis then
		for i = 1, #cluster_redis do
			local node = cluster_redis[i]
			if not node.is_myself then
				local node_client = node.redis
				request.multibulk(node_client, 'QUIT')
				node_client.network.socket:shutdown()
			end
		end
	end
...
client_prototype.shutdown = function(client)
	...
	if cluster_redis then
		for i = 1, #cluster_redis do
			...
			if not node.is_myself then
				local node_client = node.redis
				request.multibulk(node_client, 'SHUTDOWN')
				node_client.network.socket:shutdown()
			end
		end
	end
...
function redis.connect(host, port, password, cluster_support, timeout)
	local parameters = { host = host, port = port, password = password, timeout = tonumber(timeout) }
	...
	if cluster_support then
		local redis_util = require("redis_util")
		client.crc16 = redis_util.crc16
		client.cluster_support = true
