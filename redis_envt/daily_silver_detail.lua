...
function get_day_after(date, before)
	...
	if date == "2021-11-11" and not before then
		local bg = bigint_new()
		bg = bigint_add(bg, 0)
		return bg
	end
	...
	local sql = string.format("select %s from daily_silver_details where date='%s' limit 1", field, time.date(query_date, true))
...
function one_day(date, before)
	...
	local sql = string.format("insert into daily_silver_details(`date`, `origin`, `add`, `use`, `last`) values('%s', %s, %s, %s, %s)", date, bigint_tostring(bg_origin), bigint_tostring(bg_add), bigint_tostring(bg_use), bigint_tostring(bg_last))
